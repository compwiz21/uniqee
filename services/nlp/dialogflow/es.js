...
let query = async (body) => {
    ...
    let sessionPath = ''
    let sessionClient = {}
    const projectID = process.env.DIALOGFLOW_CONFIG_PROJECTID
    const agentLocation = process.env.DIALOGFLOW_CONFIG_AGENTLOCATION
    ...
    if (agentLocation) {
        ...
        sessionPath = sessionClient.projectLocationAgentSessionPath(projectID, agentLocation, platformSessionId)
    } else {
        sessionClient = new dialogflow.SessionsClient({
            credentials: credentialsJSON,
        })
        sessionPath = sessionClient.projectAgentSessionPath(projectID, platformSessionId)
    }
